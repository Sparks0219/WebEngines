...
def Simple9(postingList):
    ...
    while i < len(postingList):
        ...
        if (len(deltaLst[i::]) >= 28 and max(deltaLst[i:i+28]) <= 1):
            ...
            i+=14
        elif (len(deltaLst[i::]) >= 9 and max(deltaLst[i:i+9]) <= 7):
            i+=9
        elif (len(deltaLst[i::]) >= 7 and max(deltaLst[i:i+7]) <= 15):
            i+=7
        elif (len(postingList[i::]) >= 5 and max(deltaLst[i:i+5]) <= 31):
            i+=5
        elif (len(deltaLst[i::]) >= 4 and max(deltaLst[i:i+4]) <= 127):
            i+=4
        elif (len(deltaLst[i::]) >= 3 and max(deltaLst[i:i+3]) <= 511):
            i+=3
        elif (len(deltaLst[i::]) >= 2 and max(deltaLst[i:i+2]) <= 16383):
            i+=2 
